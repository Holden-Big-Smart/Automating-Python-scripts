            ...
            try:
                ...
                try:
                    tgt_cell.alignment = Alignment(
                        ...
                        wrap_text=a.wrapText if hasattr(a, "wrapText") else a.wrap_text,
                        shrink_to_fit=a.shrinkToFit if hasattr(a, "shrinkToFit") else a.shrink_to_fit,
                        indent=a.indent
                    )
                except Exception:
                    pass
...
def main():
    ...
    for staff in all_staff:
        if pd.isna(staff) or str(staff).strip() == "":
            continue
        course_total = df_course_valid[df_course_valid["經辨人員"] == staff]["總額"].sum() if "總額" in df_course_valid.columns else 0.0
        fee_total = df_fee_valid[df_fee_valid["經辨人員"] == staff]["會費"].sum() if "會費" in df_fee_valid.columns else 0.0
    ...
    print("✅ 已生成“最终输出”工作表，并置于最前；三个来源工作表按顺序完整拼接，表间空一行；"
          "‘統計結果’复制后再执行逐行 1~7 列合并，且样式复制采用安全模式，避免 StyleProxy 异常。")
