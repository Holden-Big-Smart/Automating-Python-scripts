...
def parse_date_input(user_input):
    """
    将输入格式 '30.10-9.11' 转换为真实日期区间，并返回：
    - start_date: datetime对象
    - end_date: datetime对象
    - excel_var: '30/10/2025-9/11/2025'
    - display_str: '30.10-9.11.2025'
    - filename_str: '30.10-9.11.2025.xlsx'
    自动判断跨年。
    """
    ...
    try:
        ...
        sd, sm = start_str.split('.')
        ed, em = end_str.split('.')

        sd, sm, ed, em = int(sd), int(sm), int(ed), int(em)
        ...
        # 输出字符串
        display_year = end_year
        display_str = f"{sd}.{sm}-{ed}.{em}.{display_year}"
        filename_str = f"{sd}.{sm}-{ed}.{em}.{display_year}.xlsx"

        return start_date, end_date, excel_var, display_str, filename_str

    except Exception as e:
        print("❌ 输入格式错误，请使用格式如：30.10-9.11")
        raise e


# 获取用户输入
user_input = input("请输入数据时间（格式：30.10-9.11）：").strip()
start_date, end_date, excel_var, display_str, filename_str = parse_date_input(user_input)

print(f"数据时间为：{excel_var}")
print(f"Excel输出时日期：{display_str}")
print(f"Excel输出文件名：{filename_str}")
