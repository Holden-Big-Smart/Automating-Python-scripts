...
def parse_date_input(user_input: str):
    ...
    left, right = s.split('-', 1)        # left: 'dd.mm'  right: 'dd.mm.yyyy'
    # 右侧必须有 年
    ...
    try:
        ...
        raise ValueError(f"无效日期：{e}")

    # 生成显示/文件名/Excel变量文本（不补零）
...
def main():
    ...
    # (2.3) 按「經辨人員」字母顺序降序排列（扩展选区的效果在 DataFrame 排序里天然满足）
    ...
    # (2.5) 按 經辨人員 / 付款方式 分组统计
    if "經辨人員" not in df_course.columns:
        raise KeyError("課程收據缺少「經辨人員」列")
    if "付款方式" not in df_course.columns:
        raise KeyError("課程收據缺少「付款方式」列")
    ...
    # (3.5) 分组统计
    if "經辨人員" not in df_fee.columns:
        raise KeyError("會費收據缺少「經辨人員」列")
    if "付款方式" not in df_fee.columns:
        raise KeyError("會費收據缺少「付款方式」列")
    ...
    # 5) 导出 Excel（三个 sheet：課程收據 / 會費收據 / 統計結果 + 數據信息）
